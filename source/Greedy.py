import sys
import bisect
from grafos import ler_arquivo
import json
import time


def heuristica(estado):

	value = 0

	for i in range(9):
		if i != estado[i]:
			value = value + 1

	return value


def heuristica_2(estado):
	value = 0
	i = 0
	for point in state:
		if point != 0:
			value = value + math.floor(math.fabs((point-i)/3)) + int(math.fabs((point-i))%3)
		i = i + 1
	return value


class EightPuzzleGREEDY(object):
	"""docstring for EightPuzzle"""
	def __init__(self, state, index=[], mov=None, cor=0, heuristic=heuristica):
		self.state = state
		self.index = index
		self.cor = cor
		self.mov = mov
		self.valor = heuristic(self.state)

	def __lt__(self, other):
		return self.valor < other.valor

def test_goal(node):
	goal = (1, 2, 3, 4, 5, 6, 7, 8, 0)
	if node.state == goal:
		return True
	else:
		return False

def Greedy(inicial, lista):
	
	frontier = []
	goal_state = None

	if test_goal(inicial):
		return 0, inicial, frontier

	frontier.append(inicial)

	i = 0
	while True:
		
		
		node = frontier[0]			
		childs = node.index
		for child in childs:
			if (lista[child[2]].cor == 0):
				lista[child[2]].mov = child
				if not test_goal(lista[child[2]]):
					i = i + 1
					lista[child[2]].cor = 1
					bisect.insort(frontier, lista[child[2]])
				else:
					#print("Final move"+str(child))
					print("Nós expandidos: " + str(i+1))
					goal_state = lista[child[2]]
					return goal_state, frontier

		node.cor = 2
		#print(len(frontier))
		frontier.remove(node)			

		#print(deep)

def gera_caminho(inicial_state, grafo, heurist):


	lista, permutaciones = ler_arquivo(Obj=EightPuzzleGREEDY, grafo=grafo, heristi=heurist)
	goal_state = None
	indice = permutaciones.index(inicial_state)
	inicial = lista[indice]

	inicio = time.time()
	goal_state, frontier = Greedy(inicial, lista)

	print("Alocação de memória do Greedy da Fronteira: "+str(sys.getsizeof(frontier)))

	node = goal_state
	caminho = []
	deep = 1

	while node.state != inicial_state:
		#print(node.mov)
		if node.mov[0] == 3:
			move = "Esquerda"
		elif node.mov[0] == 2:
			move = "Baixo"
		if node.mov[0] == 1:
			move = "Direita"
		elif node.mov[0] == 0:
			move = "Cima"
		caminho.insert(0,move)

		node = lista[node.mov[1]]

		if (node.state != inicial_state):
			deep = deep + 1

	final = time.time()

	print("Profundidade: "+str(deep))
	print("Tempo gasto: "+str(final-inicio))
	print(caminho)

if __name__ == "__main__":
	#grafo = cria_grafo(9)
	arquivo = open('lista.json', 'r')
	grafo = json.load(arquivo)
	gera_caminho((8, 6, 7, 2, 5, 4, 3, 0, 1), grafo, heurist=heuristica)